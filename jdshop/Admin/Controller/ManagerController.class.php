...
class ManagerController extends AdminController {
    ...
    function checkVerify(){
        ...
        if($this->auth_my_code($very,strtoupper($code)) == $secode['verify_code']) {
            echo json_encode(array('flag'=>1,'cont'=>'验证码正确'));die;
        }else{
    ...
    function checkUsername(){
        ...
        $info= M('Manager')->where(array('mg_name' => $username))->find();
        if($info){
            // print_r($info);exit();
            echo json_encode(array('flag'=>1,'cont'=>'用户名正确'));
            exit();
        }else{
            echo json_encode(array('flag'=>2,'cont'=>'用户名错误'));
        }
    }
}
