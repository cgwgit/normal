...
$this->page = $show;
//查询订单
$amountrecord = M('tb_amountrecord')->where(array("uid" => $id))->where("isdelete=0 and type!=6")->order('declaration_date desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
...
//        var_dump($tt);exit;
        ...
        $url1=C("URL")."integral/integral_getStepInfo.do";
        ...
        $stepinfo=$result['data'];
        $remark=$result['data'][0]['step_value']*$remarks['value'];
//        var_dump($equity);exit;
        $coin=$remark*$equity['equity'];
        $this->coin=$coin;
        $this->remark=$remark;
...
$url2=C("URL")."integral/queryChargeBackDesc.do";
...
$this->display();
 ...
 public function agree(){
    $token = $_SESSION['token'];
        ...
        $url = C("URL") . "securities/BalanceWithdrawCash.do";
...
$this->error("购买积分包数超过可购买数量");
}
$declarationNum = $declaration_num;
$token = $_SESSION['token'];
$type = 3;
//退单信息接口 获取退单信息
$dd = array(
'token' => $token, 'type' => $type, 'declarationNum' => $declarationNum, 'declarationPrice' => $amount, 'boxtotal' => $value, 'equity' => $equity
);
$dd = json_encode($dd);
//var_dump($dd);exit;
$length = strlen($dd);
...
$url3=C("URL")."integral/addOrder.do";
...
$url4=C("URL")."integral/accountChargeBack.do";
$ch = curl_init($url4);  //初始化curl
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
curl_setopt($ch, CURLOPT_POST, 1);
...
$url5=C("URL")."integral/removeChargeBack.do";
$ch = curl_init($url5);  //初始化curl
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
curl_setopt($ch, CURLOPT_POST, 1);
...
$url6=C("URL_Z")."integral_deleteOrderByEid";
$ch = curl_init($url6);  //初始化curl
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
curl_setopt($ch, CURLOPT_POST, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
